...
setTimeout(function() {hideGameMsg();}, 4000);
    ...
    btnDisabled(btn); // disable the start button since the gam is now afoot
    var btn = document.getElementById('btnStop');
...
// propsed avatar assignments and prevents them from being the same.
...
function determineAvatar() {
    // determine the correct avatar to paint for the active palyer
...
//square has already been assigned and if it has not, record new square new square with the assigned avatar.
...
//call this function to produce the following winning process for the gate
function winner(winDetected,winCon) {
    if(winDetected == "win") {
        var showme = winDetected;
        var activePlayer = document.getElementById('showPlayer').innerHTML;
        var txt2 = "That's a three in a row, "+activePlayer+" wins!";
        writeMsg(txt2);
        var btn = document.getElementById('btnStart');
        startEnabled(btn); // enable the start button since the game is now stopped
        var btn = document.getElementById('btnStop');
...
//-------------------------------------------------------------------------------------------------------
// These block of functions are for each click event fo their corresponding square element
//-------------------------------------------------------------------------------------------------------
function square1Animate() {
    var activePlayer = document.getElementById('showPlayer').innerHTML;
    if (activePlayer != "Game Stopped") { // fi the game has not yet started prevent avatar placement
        ...
        // build new array adding newly selected square and the assigned avatar
